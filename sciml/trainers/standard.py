...
#####################################################################################
class Standard(TrainerBase):
    """
    Standard training loop for PINN models, covering the majority of
    problems.
    ...
    def __init__(
        ...
        optimizer : torch.optim.Optimizer
            Optimizer used to update ``network``'s parameters. Works with
    ...
    def fit(
        ...
        if not self._started:
            ...
            if self.logger:
                self.logger.on_train_start(network=self.network, optimizers=[self.optimizer])
